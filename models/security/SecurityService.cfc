...
component output="false" singleton="true"  {
	...
	*/
	public boolean function checkUsernameAndPassword(username,password) {
		...
		if (
			...
			sessionStorage.set( "step1Valid",true );
			var user = userService.get( q.user_id );
			sessionStorage.set( "user", user );

			if ( user.isTwoFactorAuthenticationEnabled() ) {
				sessionStorage.set( "step2Required",true );
			}
			else {
	...
	*/
	public any function isLoggedIn() {
		return sessionStorage.get("isLoggedIn",false);
	}
	...
	*/
	public any function isStep1Valid() {
		return sessionStorage.get("step1Valid",false);
	}
	...
	*/
	public any function isStep2Required() {
		return sessionStorage.get("step2Required",false);
	}
	...
	*/
	public any function getLoggedInUser() {
		return sessionStorage.get("user",userService.get());
	}
